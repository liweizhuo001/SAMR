...
//	}
	...
	public boolean hasFullRank() {
		...
		// }
		int mn = Math.min(m, n);
		double radqmxn = Math.sqrt(m * n);
		// double threshold = ALG.property().tolerance();
		//double threshold = radqmxn * Utils.getDoubleMachineEpsilon();
		double threshold = Utils.getDoubleMachineEpsilon();
		//double minRjj = Double.POSITIVE_INFINITY;
		Dcs NU = N.U;//this is the equivalent of R
		for (int j = 0; j < mn; j++) {
			// if (R.getQuick(j, j) == 0){
			double rjj = getQuick(NU, j, j);
			// log.debug("r(" + j + "," + j + "): " + rjj);
			// if (rjj < Utils.getDoubleMachineEpsilon()) {
			// return false;
			// }
			//minRjj = Math.min(minRjj, rjj);
			if (Math.abs(rjj) < threshold) {
				log.debug("rjj: " + rjj);
				return false;
			}
		}
		//log.info("minRjj: " + minRjj);
		return true;
	}
