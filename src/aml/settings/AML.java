...
import aml.filter.RepairMapGraph;
import aml.filter.Repairer;
import aml.filter.RepairerGraph;
...
//Attributes
	...
	private RepairMapGraph repGraph;
...
//Public Methods
	...
	public RepairMapGraph buildRepairMapGraph()
    {
		repGraph = new RepairMapGraph();
		return repGraph;
    }
	...
	public void evaluate()
	{
		...
		if(gui)
		{
			...
			evaluation = "P\tR\tF\tFound\tCorr\tRef\n" + prc +
				"\t" + rec + "\t" + fms + "\t" + found + "\t" + correct + "\t" + total;
	}
	...
	public void evaluate2()
	{
		boolean gui = userInterface != null;
		int found = a.size();
		int correct = a.evaluate2(ref); //���Կ��ǽ�ƥ����unknown�������Ҳ��Ϊ��ȷ��������
		int total = ref.size();
	...
	public void simpleEvaluate()
	{
		...
		for(Mapping m : a.getMappingSet())
		{
			...
			else if(ref.contains(m.getSourceId(),m.getTargetId(),MappingRelation.EQUIVALENCE))
			{
				count++;
			}
			else if(ref.contains(m.getTargetId(),m.getSourceId(),MappingRelation.EQUIVALENCE))
			{
				count++;
			}
		}
		System.out.println("The number of correct mapping in saved set is "+ count);		
		int referenceNum=ref.size();
		precision = 1.0*count/a.getMappingSet().size();
		String prc = Math.round(precision*1000)/10.0 + "%";
		recall = 1.0*count/referenceNum;
		String rec = Math.round(recall*1000)/10.0 + "%";
		fMeasure = 2*precision*recall/(precision+recall);
		String fms = Math.round(fMeasure*1000)/10.0 + "%";
		
		System.out.println("P\tR\tF\t" + prc +"\t" + rec + "\t" + fms );
	}
	...
	public RepairMapGraph getRepairGraph()
	{
		return repGraph;
	}
	...
	public void repairByGraph()
	{
		im = new InteractionManager();
		repGraph = new RepairMapGraph();
		RepairerGraph r = new RepairerGraph();	
		
		r.obtainReliableMappings();

		//r.One2OneRevisedMappings();		
		//r.ChristianToolRevisedMappings();	
		//r.StrongRevisedMappingsCompleteByStrutureFunction();
		//r.StrongRevisedMappingsCompleteByOptimalFunction();
